	...
	namespace {
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				double result = EvaluateHelper(sheet);
		...
		private:
			...
			double EvaluateHelper(const SheetInterface& sheet) const {
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				...
				return type_ == UnaryMinus ? operand * -1 : operand;
			}
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				...
				Position* pos_ptr = const_cast<Position*>(cell_);

				// нужна обработка, что там пусто
				if (sheet.GetCell(*pos_ptr) == nullptr) {
					SheetInterface& non_const_sheet = const_cast<SheetInterface&>(sheet);
					non_const_sheet.SetCell(*pos_ptr, ""s);
				}

				const CellInterface* cell = sheet.GetCell(*pos_ptr);
				...
				if (auto p = std::get_if<std::string>(&val)) {
					...
					std::smatch sm;
					std::regex_search(*p, sm, re_digits);

					if (sm.size()) {
						return std::stod(sm[0]);
					}

					throw FormulaError{ FormulaError::Category::Value };
				}

				auto p = std::get_if<FormulaError>(&val);
				throw* p;
			}
