...
public:
	...
	// Обратите внимание, что в метод Evaluate() ссылка на таблицу передаётся в качестве аргумента.
	// Возвращает вычисленное значение формулы для переданного листа либо ошибку.
	// Если вычисление какой-то из указанных в формуле ячеек приводит к ошибке, то возвращается
	// именно эта ошибка. Если таких ошибок несколько, возвращается любая.
	virtual Value Evaluate(const SheetInterface& sheet) const = 0;

	// Возвращает выражение, которое описывает формулу. Не содержит пробелов и лишних скобок.
